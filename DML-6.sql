...
DROP SEQUENCE studyCommentSeq;
DROP SEQUENCE studyBoardSeq;
DROP SEQUENCE studySeq;
DROP SEQUENCE chatSeq;

CREATE SEQUENCE studyBoardSeq;
...
CREATE TABLE tblStudyBoard (
    studyBoardSeq number PRIMARY KEY,
    crtSeq number,
    id VARCHAR2(100),
    ...
    CONSTRAINT tblBoard_crtSeq foreign key(crtSeq) references tblCrt(crtSeq),
    CONSTRAINT tblBoard_id foreign key(id) references tblUserInfo(id)
);

CREATE TABLE tblStudyComment (
    studyCommentSeq number PRIMARY KEY,
    studyBoardSeq number,
    id VARCHAR2(100), 
    content VARCHAR2(3000),
    writeDate date default sysdate,
    
    CONSTRAINT tblStudyComment_studyBoardSeq foreign key(studyBoardSeq) references tblStudyBoard(studyBoardSeq),
    CONSTRAINT tblStudyComment_id foreign key(id) references tblUserInfo(id)
);
...
CREATE TABLE tblStudy (
    ...
    CONSTRAINT tblStudy_crtSeq foreign key(crtSeq) references tblCrt(crtSeq)
);

CREATE TABLE tblStudyMember (
    studyMemberSeq number PRIMARY KEY,
    studySeq number,
    id VARCHAR2(100),
    
    CONSTRAINT tblStudyMember_studySeq foreign key(studySeq) references tblStudy(studySeq),
    CONSTRAINT tblStudyMember_id foreign key(id) references tblUserInfo(id)
);
...
----------더미
insert into tblStudyBoard (studyBoardSeq, crtSeq, id, boardTitle, boardContent, boardStatus, boardHits, writeDate)
    values (studyBoardSeq.nextVal, 6120, 'N7sBxUcT', '스터디원 모집', '아무나 참여가능', 1, 20, sysdate);

--참여자 : G4yKwCtQ, N7sBxUcT
insert into tblStudyBoard (studyBoardSeq, crtSeq, id, boardTitle, boardContent, boardStatus, boardHits, writeDate)
    values (studyBoardSeq.nextVal, 2571, 'N7sBxUcT', '스터디원 모집함', '아무나 참여가능', 2, 20, sysdate);

insert into tblStudyComment (studyCommentSeq, studyBoardSeq, id, content, writeDate)
    values (studyCommentSeq.nextVal, 2, 'G4yKwCtQ', '저용', sysdate);
insert into tblStudyComment (studyCommentSeq, studyBoardSeq, id, content, writeDate)
    values (studyCommentSeq.nextVal, 2, 'pF6iLcZ', '나용', sysdate);

insert into tblStudy (studySeq, studyName, studyStatus, crtSeq, createDate)
    values (studySeq.nextVal, '지게차따기', 1, 2571, sysdate);

insert into tblStudyMember (studyMemberSeq, studySeq, id)
    values (studyMemberSeq.nextVal, 1, 'N7sBxUcT');
insert into tblStudyMember (studyMemberSeq, studySeq, id)
    values (studyMemberSeq.nextVal, 1, 'G4yKwCtQ');
insert into tblStudyMember (studyMemberSeq, studySeq, id)
    values (studyMemberSeq.nextVal, 1, 'pF6iLcZ');
...
CREATE TABLE tblChat (
    ...
    CONSTRAINT tblChat_id foreign key(sender) references tblUserInfo(id)
);
