				...
				<div class="col-lg-12 col-12 text-center">
					...
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb justify-content-center">
							...
							<li class="breadcrumb-item active" aria-current="page"><a href="/jr/crt/crtboardlist.do?crtSeq=${dto.crtSeq}">${name}</a></li>
						</ol>
					</nav>
				</div>
	...
	<section class="job-section section-padding pb-0 pt-4">
		<div class="container">
			<div class="row">
				...
				<div class="col-12">
					<h2 class="job-title mb-0 text-break" id="board-title">${ dto.crtBoardTitle }</h2>
...
function loadDetail() {
	$.ajax({
		...
		success: cbdto => {
			...
			if (cbdto) {
				$('#id').text(cbdto.id);
				$('#board-write-date').text(cbdto.crtboardWriteDate);
				$('#board-like').text(cbdto.crtBoardLike);
				$('#board-hits').text(cbdto.crtBoardHits);
				$('#board-report').text(cbdto.crtBoardReport);
			} else { // 반환이 제대로 안됨
		...
		error: (a,b,c) => {
			console.log(a,b,c);
						...
						<div
							...
							<a href="/jr/crt/crtboardlist.do?crtSeq=${dto.crtSeq}" class="custom-btn btn mt-2 mb-5">목록</a>
							<c:if test="${ id == dto.id }">
								<a href="/jr/crt/edit.do?crtBoardSeq=${ dto.crtBoardSeq }"
		...
		.click(
				function() {
					if (confirm('삭제 후 되돌릴 수 없습니다. 삭제하시겠습니까?')) {
						location.href = '/jr/crt/del.do?crtBoardSeq=${dto.crtBoardSeq}';
					}
				});
</script>
						</div>
...
<c:if test="${dto.isCrtBoardShow == 0}">
...
function loadComment() { 
	...
	$.ajax({
		type: 'GET',
		url: '/jr/crt/comment',
		data: 'crtBoardSeq=${dto.crtBoardSeq}',
		dataType: 'json',
		success: list => { // 댓글 목록

			<c:if test="${dto.isCrtBoardShow != 1}">
			return;
			</c:if>
				...
				let temp = `	
					<div class="comment d-flex ps-3 pe-3 pt-3 pb-3 border-bottom">
						<div>
							...
							<div class="comment-content pt-3 pb-3 text-break">\${item.crtCommentContent}</div>									
						  	<div class="comment-regdate">\${item.crtCommentWDate}</div>
						</div>
				`;
					...
					temp += `
				  	<c:if test="${not empty id}">
						<div class="d-flex ms-auto align-items-center">
                          <p class="mb-0">
                              <a href="javascript:void(0);" onclick="editComment(\${item.crtCommentSeq})" class="badge">수정</a>
                          </p>

                          <p class="mb-0">
                              <a href="javascript:delComment(\${item.crtCommentSeq});" class="badge badge-level">삭제</a>
                          </p>
                      	</div>
					</c:if>
						`;
...
function delComment(commentSeq) {
	...
	if (confirm('삭제 후 되돌릴 수 없습니다. 삭제하시겠습니까?')) {
			$.ajax({
				type: 'DELETE',
				url: '/jr/crt/comment/' + commentSeq.toString(),
				dataType: 'json',
...
function setLikeBtn() {
	...
	/* 좋아요 눌렀는지 확인 */
	let liked = '';
	<c:if test="${ not empty liked && dto.isCrtBoardShow == 1}">
		liked = 'y';
	</c:if>
