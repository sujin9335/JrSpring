...
.navbar-vertical.bg-white .navbar-nav>.nav-item .nav-link.board {
	box-shadow: none;
	background-color: #f6f9fc;
}

.navbar-vertical .navbar-nav .nav-link.board {
	font-weight: 600;
...
<body class="g-sidenav-show   bg-gray-100">
								...
								<table class="table align-items-center mb-0">
									<thead>
										<tr>
											...
											<th
												class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">제목</th>
											<th
											...
											<th
												class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">비공개</th>
										</tr>
									...
									<tbody>
										<c:forEach items="${list}" var="dto" varStatus="status">
											<tr>
												<td>${dto.boardSeq}</td>
												<td><a
													href="/jr/admin/board/boardadmindetail.do?boardSeq=${dto.boardSeq}">${dto.boardTitle}</a></td>
												<td>${dto.id}</td>
												<td>${dto.boardWriteDate}</td>
												<td><c:choose>
														<c:when test="${dto.isBoardShow == 1}">
															<input class="Private" type="checkbox">
														</c:when>
														<c:when test="${dto.isBoardShow == 0}">
															<input class="Private" type="checkbox" checked>
														</c:when>
														<c:otherwise>
	...
	$('.Private').on('change', function() {
	    ...
	    // 체크박스 상태에 따라 다른 데이터를 전송하는 Ajax 요청
	    $.ajax({
	        type: 'POST',
	    ...
	    // 만약 체크박스가 선택 해제되었다면 다른 AJAX 요청을 보냄
	    if (!isChecked) {
	        // 다른 백엔드 엔드포인트 및 데이터를 사용
	        $.ajax({
	            type: 'POST',
	            url: '/jr/admin/boardpublic.do', // 다른 백엔드 엔드포인트
	            data: {
	                isChecked: isChecked,
	                boardSeq: boardSeq
	            },
