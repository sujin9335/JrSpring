 ...
 */
@Service
public class UserAdminPageService {
   ...
   public void paging(AdminPageDTO pdto) {
      ...
      //총 글의 개수
      pdto.setTotalCount(dao.userGetTotalCount(pdto));
      ...
      if (n == 1) {
         ...
         if (pdto.getSearch() == null) {
            sb.append(String.format("<li class='page-item'><a class='page-link' href='/jr/admin/user/useradminlist.do?page=%d'\n"
                  + "   aria-label='Previous'> <span aria-hidden=\"true\">Prev</span>\n"
                  + "   </a></li>", n - 1));
         } else {
            sb.append(String.format("<li class='page-item'><a class='page-link' href='/jr/admin/user/useradminlist.do?page=%d&column=%s&word=%s'\n"
                  + "   aria-label='Previous'> <span aria-hidden=\"true\">Prev</span>\n"
      ...
      while (!(loop > blockSize || n > pdto.getTotalPage())) {
         ...
         if (n == pdto.getPage()) {
            ...
            if (pdto.getSearch() == null) {
               sb.append(String.format("<li class='page-item'><a class='page-link' href='/jr/admin/user/useradminlist.do?page=%d'>%d</a></li>", n, n));
            } else {
               sb.append(String.format("<li class='page-item'><a class='page-link' href='/jr/admin/user/useradminlist.do?page=%d&column=%s&word=%s'>%d</a></li>", n,
      ...
      if (n > pdto.getTotalPage()) {
         ...
         if (pdto.getSearch() == null) {
            ...
            sb.append(String.format(" <a href='/jr/admin/user/useradminlist.do?page=%d&column=%s&word=%s'>[다음]</a>",
                  pdto.getColumn(), pdto.getWord(), n));
            sb.append(String.format("<li class='page-item'><a class='page-link' href='/jr/admin/user/useradminlist?page=%d&column=%s&word=%s'\n"
                  + "                     aria-label='Next'> <span aria-hidden='true'>Next</span>\n"
                  + "                  </a></li>",
